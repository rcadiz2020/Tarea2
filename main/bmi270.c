...
const uint8_t bmi270_config_file[] = {
    ...
    0x30, 0xf4, 0x7f, 0xd0, 0x7f, 0xb2, 0x7f, 0xe3, 0x30, 0xc5, 0x6f, 0x56, 0x40, 0x45, 0x41, 0x28, 0x08, 0x03, 0x14,0x0e, 0xb4, 0x08, 0xbc, 0x82, 0x40, 0x10, 0x0a, 0x2f, 0x54, 0x26, 0x05, 0x91, 0x7f, 0x44, 0x28, 0xa3, 0x7f, 0x98,
    ...
    0x30, 0x21, 0x2e, 0x95, 0x01, 0x45, 0x6f, 0x2a, 0x8d, 0xd2, 0x7f, 0xcb, 0x7f, 0x13, 0x2f, 0x02, 0x30, 0x3f, 0x50,0xd2, 0x7f, 0xa8, 0x0e, 0x0e, 0x2f, 0xc0, 0x6f, 0x53, 0x54, 0x02, 0x00, 0x51, 0x54, 0x42, 0x0e, 0x10, 0x30, 0x59,
...
static esp_err_t bmi_read(uint8_t *data_address, uint8_t *data_rd, size_t size) {
...
static esp_err_t bmi_write(uint8_t *data_address, uint8_t *data_wr, size_t size) {
    esp_err_t ret;

    uint8_t *full_data = (uint8_t *)malloc(1 + size);
    if (full_data == NULL) {
        ESP_LOGE(TAG, "Fallo al reservar %d bytes de memoria en bmi_write", 1 + size);
        return ESP_ERR_NO_MEM; // Error si no hay memoria
    }
...
static esp_err_t i2c_bus_init(void) {
    ...
    return i2c_master_bus_add_device(bus_handle, &dev_cfg, &device);
}

static void chipid(void) {
    uint8_t reg_id=0x00, tmp;
    bmi_read(&reg_id, &tmp, 1);
    ESP_LOGI(TAG, "Valor de CHIPID: %2X", tmp);
    if(tmp != 0x24) {
        ESP_LOGE(TAG, "Chip no reconocido. CHIP ID: %2x", tmp);
        // exit(EXIT_SUCCESS); // Mejor no usar exit() en un micro
    } else {
        ESP_LOGI(TAG, "Chip reconocido.");
    }
}

static void softreset(void) {
    uint8_t reg_softreset=0x7E, val_softreset = 0xB6;
    esp_err_t ret = bmi_write(&reg_softreset, &val_softreset, 1);
    vTaskDelay(1000 /portTICK_PERIOD_MS);
    if(ret == ESP_OK) ESP_LOGI(TAG, "Softreset: OK");
    else ESP_LOGE(TAG, "Error en softreset: %s", esp_err_to_name(ret));
}
...
static void initialization(void) {
    ...
    bmi_write(&reg_init_ctrl, &val_init_ctrl,1);
    
    int config_size = sizeof(bmi270_config_file);
    esp_err_t ret = bmi_write(&reg_init_data, (uint8_t*)bmi270_config_file, config_size);
    if(ret != ESP_OK) ESP_LOGE(TAG, "Error cargando config_file");
    else ESP_LOGI(TAG, "Config_file cargado.");
    
    vTaskDelay(1000 /portTICK_PERIOD_MS);
    bmi_write(&reg_init_ctrl, &val_init_ctrl2,1);
    ESP_LOGI(TAG, "Algoritmo de inicializacion finalizado.");
}

static void check_initialization(void){
    uint8_t reg_internalstatus=0x21, tmp;
    vTaskDelay(1000 /portTICK_PERIOD_MS);
    bmi_read(&reg_internalstatus, &tmp, 1);
    if((tmp & 0b00001111)==1) {
        ESP_LOGI(TAG, "Comprobacion Inicializacion: OK");
    } else {
        ESP_LOGE(TAG, "Inicializacion fallida (status: %x)", tmp);
    }
}
...
static void internal_status(void) {
    uint8_t reg_internalstatus=0x21, tmp;
    bmi_read(&reg_internalstatus, &tmp, 1);
    ESP_LOGI(TAG, "Internal Status: %2X", tmp);
}

static void bmipowermode(void) {
    uint8_t reg_pwr_ctrl=0x7D, val_pwr_ctrl=0x04; // ACC on
    uint8_t reg_gyr_conf=0x42, val_gyr_conf=0xAA; // GYR 800Hz, +/-2000dps
    uint8_t reg_pwr_ctrl_gyr=0x7D, val_pwr_ctrl_gyr=0x0E; // GYR on
    ...
    if(Fodr==200 ) val_acc_conf=0xA9;
        ...
        ESP_LOGW(TAG, "Frecuencia ODR no válida, usando 800Hz");
        val_acc_conf=0xAB;
    };
    
    bmi_write(&reg_acc_conf, &val_acc_conf, 1); // ACC Config
    bmi_write(&reg_gyr_conf, &val_gyr_conf, 1); // GYR Config
    vTaskDelay(100 / portTICK_PERIOD_MS);
    bmi_write(&reg_pwr_ctrl_gyr, &val_pwr_ctrl_gyr, 1); // ACC y GYR on
    vTaskDelay(100 / portTICK_PERIOD_MS);
    ESP_LOGI(TAG, "Sensor en modo normal (ACC+GYR).");
}



esp_err_t bmi270_init(void) {
    ESP_RETURN_ON_ERROR(i2c_bus_init(), TAG, "Fallo al iniciar bus I2C");
    softreset();
    chipid();
    initialization();
    ...
    bmipowermode();
    internal_status();
    ESP_LOGI(TAG, "BMI270 inicializado correctamente.");
    return ESP_OK;
}

esp_err_t bmi270_read_data(bmi270_data_t *data) {
    uint8_t reg_intstatus = 0x03, tmp;
    uint8_t reg_data = 0x0C, data_data8[12];
    uint16_t acc_x, acc_y, acc_z, gyr_x, gyr_y, gyr_z;

    // Comprobar el bit de "data ready" (DRDY_ACC)
    bmi_read(&reg_intstatus, &tmp, 1);
    
    // (tmp & 0xC0) comprueba DRDY_ACC (0x80) y DRDY_GYR (0x40)
    if ((tmp & 0x80) == 0x80) 
    {
        esp_err_t ret = bmi_read(&reg_data, data_data8, 12);
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "Error en lectura BMI: %s", esp_err_to_name(ret));
            return ret;
        }
        ...
        // Convertir a float
        // Rango ACC: 78.4532 m/s2 (8g)
        data->ax = (int16_t)acc_x * (78.4532 / 32768.0);
        data->ay = (int16_t)acc_y * (78.4532 / 32768.0);
        data->az = (int16_t)acc_z * (78.4532 / 32768.0);
        
        // Rango GYR: 34.90659 rad/s (2000dps)
        data->gx = (int16_t)gyr_x * (34.90659 / 32768.0);
        data->gy = (int16_t)gyr_y * (34.90659 / 32768.0);
        data->gz = (int16_t)gyr_z * (34.90659 / 32768.0);

        return ESP_OK; // Datos leídos
    }
    
    return ESP_ERR_NOT_FOUND; // Datos no estaban listos
}