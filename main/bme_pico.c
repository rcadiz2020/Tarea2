...
#define ALTURA_MIN_DEF   0      // valor por defecto si no se especifica (p.ej., sin filtro)
...
//detector de picos
size_t detectar_picos_simple(
    const int16_t x[N_SAMPLES],
    size_t dist_min,
    int16_t altura_min,
    int16_t picos_val[N_SAMPLES],
    size_t  picos_idx[N_SAMPLES]
) {
    if (altura_min == INT16_MIN) {
        altura_min = ALTURA_MIN_DEF; // aplica valor por defecto
    }

    size_t count = 0;
    // para aplicar distancia mínima, recordamos el último índice aceptado
    size_t last_idx = (size_t)(-1);

    // recorre evitando bordes (0 y N_SAMPLES-1 no pueden ser picos estrictos)
    for (size_t i = 1; i + 1 < N_SAMPLES; ++i) {
        // pico estricto y sobre el umbral
        if (x[i] > x[i - 1] && x[i] > x[i + 1] && x[i] >= altura_min) {
            // verifica distancia mínima respecto al último pico
            if (last_idx == (size_t)(-1) || (i - last_idx) >= dist_min) {
                if (picos_val) picos_val[count] = x[i];
                if (picos_idx) picos_idx[count] = i;
                last_idx = i;
                count++;
            } else {
                // Si no cumple distancia, opcionalmente podrías reemplazar por el mayor
                // entre el actual y el último (no implementado para mantenerlo simple)
            }
        }
    }
    return count;
...
void bme_read_and_detect_peaks(void) {
    static int16_t senal[N_SAMPLES]; // Buffer para 1000 muestras
    static size_t sample_count = 0;
    
    // Buffers para almacenar los resultados de la detección
    static int16_t picos_val[N_SAMPLES];
    static size_t  picos_idx[N_SAMPLES];

    for (;;) {
        // 1. Lectura del BME688 (igual que antes)
        uint8_t tmp;
        uint32_t temp_adc = 0;
        bme_forced_mode(); // Inicia la conversión
        
        // Espera un momento para asegurar la conversión (ajusta según el T_MEAS del BME688)
        vTaskDelay(pdMS_TO_TICKS(100)); 

        // Leer datos ADC de temperatura (registros 0x22, 0x23, 0x24)
        ...
        // Calcula la temperatura en centésimas de grado C (multiplica por 100)
        int32_t temp_c_x100 = bme_temp_celsius(temp_adc); 
        
        // 2. Almacenamiento de la muestra
        // Almacenamos la temperatura en centésimas para el detector de picos (int16_t)
        senal[sample_count] = (int16_t)temp_c_x100;
        printf("[%zu] T: %f C\n", sample_count, (float)temp_c_x100 / 100.0f); 

        sample_count++;

        // 3. Detección de picos cuando el buffer está lleno
        if (sample_count >= N_SAMPLES) {
            size_t dist_min = 5;      // Ejemplo: Mínimo 5 muestras entre picos
            int16_t altura_min = 20;  // Ejemplo: Mínimo 0.20 C (20 centésimas)

            printf("\n--- BUFFER LLENO: Ejecutando Detección de Picos ---\n");
            
            size_t n_picos = detectar_picos_simple(
                senal, 
                dist_min, 
                altura_min, 
                picos_val, 
            ...
            printf("Resultado: Se encontraron %zu picos.\n", n_picos);
            for (size_t i = 0; i < n_picos; i++) {
                printf("  Pico #%zu: Valor = %f C (en centésimas: %d), Índice = %zu\n", 
                    i + 1, (float)picos_val[i] / 100.0f, picos_val[i], picos_idx[i]);
            }
            printf("---------------------------------------------------\n\n");
            
            // Reinicia el contador para empezar a llenar el buffer de nuevo
            sample_count = 0;
        }

        // Retardo para la siguiente muestra. ¡MUY IMPORTANTE!
        // El tiempo total de muestreo debe ser (N_SAMPLES * delay)
        vTaskDelay(pdMS_TO_TICKS(100)); // Espera 100ms para la siguiente muestra
    }
}
...
void app_main(void) {
    ...
    bme_softreset();
    bme_get_mode();
    // bme_forced_mode(); <--- ELIMINADO/COMENTADO, ya se llama en cada iteración de la siguiente función.
    printf("Comienza lectura y detección de picos\n\n");
    bme_read_and_detect_peaks();
}
